...
\end{itemize}
In baseline results I focus on products classified as \textbf{High}; \textbf{Medium} only enters robustness/upper bounds.
...
\end{itemize}
The schema enforcement ensures that every response contains valid values for all fields. The model cannot return ``Medium-High'' or ``Moderate''---only the three specified relevance levels. The structured approach facilitates systematic analysis and comparison across different LLM models.
...
I employ Claude (Anthropic) as my main classification model, using the tool-calling API to obtain structured responses.\footnote{I use the \texttt{claude-sonnet-4-20250514} model. Results are robust to using alternative frontier models; see Appendix \ref{app:robustness}.} I set the temperature... I process all 19,424 HS10 codes through the classification API with rate limiting to respect API constraints. Processing all codes required about a day and a half at a total cost of approximately \$150.
